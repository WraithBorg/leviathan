...
public class LoginController {
    ...
    public Object loginsave(LoginDTO loginDTO) {
        ...
        if (v_telephone.size() == 0) {
            return CCommonUtils.ofMap("error", 1, "message", "账号不存在", "data", "", "url", CConstant.WEB_HOST+"/h5/pages/login/index");
        }
        UserInfo userInfo = v_telephone.get(0);
        if (!userInfo.getPassword().equals(loginDTO.getPassword())) {
            return CCommonUtils.ofMap("error", 1, "message", "密码不正确", "data", "", "url", CConstant.WEB_HOST+"/h5/pages/login/index");
        }
        //
        ...
        return CCommonUtils.ofMap("error", 0, "message", "登陆成功", "data", data, "url", "/index.php");
    ...
    public Object findpwdSave() {
        ...
        return CCommonUtils.ofMap("error", 0, "message", "密码修改成功，马上登录", "data", 0, "url:", "/index.php?m=login");
    }
}

package com.io.hydralisk.constant;

import java.io.File;

public class CConstant {
    public static final Long USER_ID = 1L;
    public static final String IMAGE_HOST = "https://kfbc-deitui-com.oss-cn-hangzhou.aliyuncs.com/";
    public static final String WEB_HOST = CConstant.WEB_HOST+"";
    public static final String DEFAULT_TELEPHONE = "110";
    public static final String DEFAULT_HEAD_IMG = "https://kfbc-deitui-com.oss-cn-hangzhou.aliyuncs.com//static/images/user_head.jpg";


}
